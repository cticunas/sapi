...
class ResearchController extends Controller
{
    ...
    public function public_list_new(Request $request){
        $params=$request->all();
        $list=$this->repository->public_list_new($params);
        return $list;
    }
    ...
    public function public_list_by_year(Request $request){
        ...
        $list=$this->repository->public_list_by_year($params);
    ...
    public function py_by_state(Request $request) {
        ...
        $research = array_map(function($e){
                        ...
                        return ["code"=>$e->code,"title"=>$e->title, "document"=>$e->document, "authors"=>$authors,"research_state"=>$e->research_state, "area"=>$e->area, "group"=>$e->group, "line"=>$e->line];
                    }, $list->toArray());
        if ($params['format'] == 'excel') {
            $excel = new \App\Exports\PyByStateExport(['data'=>$research, 'type_research'=>$params['type_research']]);
            return $excel->download('py_by_state.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        } else {
    ...
    public function py_by_college(Request $request){
        ...
        $research = array_map(function($e) {
            $authors = $this->repository->explode_authors($e->authors,false);
            return ["code"=>$e->code,"title"=>$e->title, "document"=>$e->document, "authors"=>$authors,"organization_id"=>$e->organization_id,'area'=>$e->area,'group'=>$e->group,'line'=>$e->line];
        },$list->toArray());
        ...
        if($params['format'] == 'excel') {
            ...
            // dd($research);
            $excel = new \App\Exports\PyByCollegeExport(['data'=>$research, 'college'=>$college]);
            return $excel->download('py_by_college.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        } else {
        ...
        try {
            ...
            foreach($colleges as $c) {
                ...
                $research = array_map(function($e) {
                    ...
                    //dd($results);
                    return ["college"=>$e->college,"main_objective"=>$main_objective,"objectives"=>$objectives,"title"=>$e->title, "date_init"=>$e->date_init, "date_end"=>$e->date_end,"budget"=>$e->budget,"fin_company"=>$e->fin_company,"area"=>$e->area,"group"=>$e->group,"line"=>$e->line,"main_author"=>$main_author,"authors"=>$authors,"fin_type"=>$e->fin_type,'results'=>$results];
                }, $research_by_org);
    ...
    public function py_by_period(Request $request) {
        ...
        foreach($organizations as $o) {
            ...
            $research = array_map(function($e) {
                $authors = $this->repository->explode_authors($e->authors, false);
                return ["code"=>$e->code,"date_init"=>$e->date_init,"date_end"=>$e->date_end ,"title"=>$e->title, "log_date_at"=>$e->log_date_at, "authors"=>$authors, "area"=>$e->area, "group"=>$e->group, "line"=>$e->line];
            },$research_by_org);
        ...
        // }
        if( $params['format']=='excel') {
            $excel=new \App\Exports\PyByPeriodExport(['data'=>$data, 'school'=>$params['school'], 'type_research'=>$type_research, 'state'=>$params['research_state_id'], 'year'=>$params['year']]);
            return $excel->download('py_by_period.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        }
        else {
            return view('reports/py_by_period',['data'=>$data,'i'=>1,'year'=>$params['year'],'state'=>$params['research_state_id'],'research_state'=>$research_state,'type_research'=>$type_research, 'school'=>$params['school']]);
        }
    }
}
