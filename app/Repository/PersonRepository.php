...
class PersonRepository extends BaseRepository implements PersonRepositoryI {
  ...
  public function list_roles($params){
    ...
    public function get_author_activity($id){
        $conditions = ['research_authors.author_id'=>$id,'research.status'=>1];
        ...
        ->where($conditions)->groupBy(DB::raw("$year_date_init "));
        return $q->get();
    }
    ...
    public function get_authors($params){
        ...
        if( array_key_exists("area_id",$params) ){ $q->whereRaw("research.area_id = '$params[area_id]'");}
