<template>
    <div style="padding:10px 20px 10px 10px">
        ...
        <div style="display:flex; gap: 5px;padding:5px; background:#333; color:white; flex-wrap: wrap;">
            <div>
                <label style='margin: 0; padding-right: 5px;'>Escuela: </label>
            ...
            <div>
                <label style='margin: 0; padding-right: 5px;'>Año:</label>
            ...
            -->
            <a-button type="primary" icon="search" @click="fetch({format:'html'})" title="Buscar"></a-button>
...
export default {
    ...
    async mounted() {
        ...
        // this.fetch();
        this.filter = {type_research:1, year:moment().format('YYYY'),research_state_id:3,grade:1 };
        this.listColleges();
    },
    ...
    methods: {
        ...
        async fetch(params = {}) {
            try {
                ...
                params = { ...this.filter };
                const school = this.colleges.find( e=> e.id == params.organization_id);
                ...
                this.report_url = default_report_url+'?type_research='+params.type_research+'&year='+ params.year+'&research_state_id='+ params.research_state_id+'&organization_id='+ params.organization_id+"&format="+format+"&r="+Math.random()+(params.type_research==1?'&grade='+params.grade:'')+"&school="+school.name;
            } catch(error) {
                this.error(error);
            }
        },
