...
<div style="padding:10px 20px 10px 10px">
  <div style="display:flex">
    <h2>CONSTANCIA DE TRABAJO DE INVESTIGACION</h2>
    <div style="padding-left:1em;">
      <a-tooltip>
        <template slot="title">
          <span>Aqui se listan las investigaciones en estado 'Ejecución'. Además esas investigaciones NO deben estar marcados como 'Externas'</span>
        </template>
      ...
      <a-form-model style="display:flex; width:100%; gap: 10px;padding:8px; background:#333;color:white; align-items:center">
        <div style="display:flex">
          <label style="margin-right:.5em">Investigador: </label>
        ...
        <div style="display:flex">
          <label style="display:flex; align-items:center; margin-right:.5em">Fechas:</label>
          <div >
            <a-date-picker v-model="filter.from" placeholder="Desde" style="width:120px;margin-bottom:8px" :format="DATEFORMAT"/>
...
const default_report_url='/api/research/constancy' 
          ...
          async fetch(params={}){
             try{
               ...
               let format = params.format;
                ...
                let to = moment(params.to).format('YYYY-MM-DD');
                 this.report_url=default_report_url+'?author_id='+params.author_id+'&type_research='+params.type_research+'&from='+from+'&to='+to +'&format='+format+(params.grade>0?'&grade='+params.grade : '')+'&r='+Math.random();
              }catch(error){
                this.error(error);
            }
          },
           async word(params = {}){
              try{
                 ...
                 params.date_init=this.filter.date_init;
                 params.date_end=this.filter.date_end;
