...
<div style="padding:10px 20px 10px 10px">
  ...
  <a-row>
    <a-col :span="6">
      <h4>Investigaciones <a-button type="primary" size="small" @click="newo()">Nuevo</a-button></h4>
    </a-col>
  ...
  >
    ...
    <div slot="stateResearch" slot-scope="text,record" style="text-align:center">
      {{getStatusName(record.research_state_id)}}  <br /> <span v-if="getPendingTime(record.date_end)<0 && ! /Culminado|Anulado|Suspendido/. test(getStatusName(record.research_state_id))" style="pointer:default">
       <a-tooltip>
          <template slot="title">
    ...
    <span slot="action" slot-scope="text,record">
      ...
      <a href="javascript:;" title="Editar" v-if=" record.research_state_id != RESEARCH_STATE_ANUL && ( checkOwner(record) || user_logged.role_id == ADMIN_ROLE )" @click="edit(record.id)"><a-icon type="edit" /></a>
      ...
      <a href="javascript:;" title="Ver" v-if="record.research_state_id != RESEARCH_STATE_ANUL &&  !checkOwner(record)" @click="edit(record.id, {readOnly:true})"><a-icon type="eye" /></a>
    </span>
  </a-table>
      ...
      <a-row>
        ...
        <a-col :span="12">
          ...
          <a-form-model-item label="Facultad" prop="department">
            <a-select v-model="research.faculty_id" placeholder="Seleccione una Facultad" @change="onChangeColleges()" :disabled="research.readOnly || !research.allowEdit">
          ...
          <a-form-model-item label="Grupo" prop="group">
            <a-select show-search option-filter-prop="children" v-model="research.group_id" placeholder="Seleccione un Grupo" @change="onChangeGroup()" :disabled="research.readOnly || !research.allowEdit">
              <a-select-option v-for="item in programs" :value="item.id" :key="item.id" :title="item.name">
                {{item.name}}
              </a-select-option>
            </a-select>
          </a-form-model-item>
...
export default {
  ...
  methods:{
    ...
    async onChangeColleges(){
      try {
        this.colleges = this.faculties.find(e=>e.id == this.research.faculty_id).children.map(e=>({id:e.id,name:e.name}));
        this.research.organization_id = this.colleges[0].id;
        //await this.listPrograms(this.research.plan);
        //this.research.program_id = this.programs[0].id;
    ...
    async onChangePlan(){
      ...
      if (this.programs.length) r.group_id = this.programs[0].id;
    ...
    onChangeGroup(){
      try {
        if (!this.programs) throw ("No existe Grupo");
        const group = this.programs.find(e=>e.id==this.research.group_id);
        this.lines = group ? group.children : this.programs.length?this.programs[0].children:[];
        this.research.line_id = this.lines.length?this.lines[0].id:null;
      } catch (error) {
        this.error(error)
      }
    },
    ...
    async listPrograms(plan_id){
      ...
      let programs_by_faculty = data.filter(e=>e.organization_id == this.research.faculty_id);
      this.programs = programs_by_faculty;
    },
    getOutcomeType(id){
    ...
    async saveOutcomeFiles(){
      for( var i=0; i<this.newOutcomeFiles.length;i++){
    ...
    async saveStatusFiles(){
      for( var i=0; i<this.newStatusFiles.length;i++){
    ...
    async newo(){
      try {
        ...
        this.research.organization_id = this.user_logged.college_id;
        await this.onChangePlan();
        if(this.user_logged.group_id) this.research.group_id = this.user_logged.group_id;
        await this.onChangeGroup();
        if(this.user_logged.line_id) this.research.line_id = this.user_logged.line_id;
    ...
    async edit(id, params={}){
      ...
      this.onChangeColleges();
      this.research.organization_id = d.organization_id;
      await this.onChangePlan();
      this.research.group_id = d.group_id;
      this.onChangeGroup();
      this.research.line_id = d.line_id;
      this.showResearchModal = true;
    },
    async remove(id){
      try {
